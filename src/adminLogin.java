...
*/
    ...
    public void build() {
        try {
            ...
            Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/empmange", "vatiza", "admin");
            Statement stm = con.createStatement();
            String sql = "select * from admin where email='" + email + "' and pass=MD5('" + password + "')";
            ResultSet rs = stm.executeQuery(sql);
            if (rs.next()) {
...
// login();
    String getEmail=idField.getText();
    String getPass=passField.getText();
    
LoginBuilder builder = new LoginBuilder();

        builder.withEmail(getEmail).withPassword(getPass).build();
